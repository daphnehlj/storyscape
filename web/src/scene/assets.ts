 ...
 * `shadows: false` turns off both casting and receiving — with variance shadow maps, receivers are drawn
 * into the shadow map too, so a receiver 50 m up would still stamp a blob on the ground.
 */
export function useNormalizedModel(url: string, size: number, shadows = true): Group {
  ...
  return useMemo(() => {
    ...
    model.traverse((o) => {
      if (o instanceof Mesh) { o.castShadow = shadows; o.receiveShadow = shadows }
    })
    const g = new Group()
    g.add(model)
